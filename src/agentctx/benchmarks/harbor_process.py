...
WORKER_MODULE = "agentctx.benchmarks.harbor_adapter"
...
def worker_environment() -> dict[str, str]:
    """Make ``agentctx`` and ``minisweagent`` importable in the worker.

    Harbor may have been launched without PYTHONPATH (e.g. the cancellation
    check script or the tests), so prepend the source roots explicitly.
    """
    env = os.environ.copy()
    roots = [
        str(WORKSPACE_ROOT / "src"),
        str(WORKSPACE_ROOT / "mini-swe-agent" / "src"),
    ]
    ...
    try:
        ...
        with (logs_dir / "worker.log").open("ab") as worker_log:
            process = subprocess.Popen(
                [sys.executable, "-m", WORKER_MODULE, "--worker", str(child.fileno())],
                cwd=WORKSPACE_ROOT,
