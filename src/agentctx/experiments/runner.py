...
experiment configuration, benchmark dispatch, metadata, and result persistence.
Condition definitions live in ``agentctx.experiments.conditions``.
...
---------------------------------
  ...
  Default compressed-condition budget = 15 000 tokens; see conditions.py.
...
SUMMARY_CONFIG: Path | None = None   # CLI path; environment overrides are resolved by summary_model_info
...
def main() -> None:
    ...
    if args.summary_config:
        ...
        # Both benchmark adapters spawn agents with os.environ.copy(), so
        # setting it here reaches every agent process (primitives.get_summary_model).
    ...
    if not (args.eval_only and (model_results_dir() / "run_info.json").exists()):
        _write_run_info(conditions, len(tasks), total, budget)
