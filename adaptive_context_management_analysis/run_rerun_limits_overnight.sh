...
# First waits for any running phase-3 runner (logs/experiments/rerun_qwen35b_fc_limits_phase3.pid)
...
# The chain writes its own log (logs/experiments/rerun_qwen35b_fc_limits_overnight_<ts>.log)
# and both stages log into it as well.
set -uo pipefail
WS="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
cd "$WS"
source "$WS/scripts/lib/logpaths.sh"
export AGENTCTX_LOG_FILE="$(experiment_log rerun_qwen35b_fc_limits_overnight)"
exec > >(tee -a "$AGENTCTX_LOG_FILE") 2>&1
echo "log: $AGENTCTX_LOG_FILE"
...
P3_PID="$EXPERIMENT_LOG_DIR/rerun_qwen35b_fc_limits_phase3.pid"
