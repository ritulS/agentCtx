"""Harbor adapter worker lifecycle: normal submission, cancellation, reaping.

Requires the ``harbor`` package, so run with the Harbor interpreter:
``venv-harbor/bin/python -m pytest tests/test_harbor_cancellation.py``.
The suite is skipped under interpreters without Harbor.
"""
...
except ImportError as exc:  # pytest reports SkipTest raised at import as a skip
    raise unittest.SkipTest(f"harbor is not importable: {exc}")
...
class CancellationTests(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        ...
        # The worker subprocess must be able to import BlockingSocketModel
        # from this module by the name the test runner gave it.
        self.previous_pythonpath = os.environ.get("PYTHONPATH")
        os.environ["PYTHONPATH"] = os.pathsep.join(
            [str(TESTS_DIR)] + ([self.previous_pythonpath] if self.previous_pythonpath else [])
        )
    ...
    async def test_timeout_closes_blocking_inference_socket(self):
        ...
        async with server:
            agent = self.agent(model={
                "model_class": f"{__name__}.BlockingSocketModel",
