...
# Root-level modules that pre-reorganization trees import from the repository
# root (``summary_config.py`` by the runner, ``memory.py`` by the agent).
ROOT_MODULES = ("summary_config.py", "memory.py")


@dataclass(frozen=True)
class Tree:
    """A copy of the code to exercise, laid out like the repository root."""
    ...
    def _sources(self, *relative: str) -> list[Path]:
        """Existing files among ``relative`` paths, looked up below scripts/ and src/."""
        found = []
        for rel in relative:
            found.extend(p for p in (self.scripts_dir / rel, self.root / "src" / rel) if p.is_file())
        return found

    def _has(self, *relative: str) -> bool:
        return bool(self._sources(*relative))

    def _mentions(self, needle: str, *relative: str) -> bool:
        return any(needle in path.read_text() for path in self._sources(*relative))

    @property
    def features(self) -> frozenset[str]:
        """Capabilities detected from the tree, used to skip scenarios it predates."""
        features = set()
        if self._has("bench_adapters/terminal_bench.py", "agentctx/benchmarks/terminal_bench.py"):
            features.add("terminal-bench")
        if self._has("bench_adapters/tb_verdict.py", "agentctx/benchmarks/tb_verdict.py"):
            features.add("tb-verdicts")
        runners = ("run_experiment.py", "agentctx/experiments/runner.py")
        if self._mentions("--summary-config", *runners):
            features.add("summary-config")
        iclr = ("run_experiment_iclr.py", "agentctx/experiments/iclr.py")
        if self._mentions("prefix_cache_ablation", *iclr):
            features.add("iclr-sections")
        if self._has("run_budget_calibration_tb.py", "calibration/run_budget_calibration_tb.py"):
            features.add("tb-calibration")
        if self._has("run_budget_calibration_sb.py", "calibration/run_budget_calibration_sb.py"):
            features.add("sb-calibration")
        return frozenset(features)


def current_tree() -> Tree:
    return Tree(label="working-tree", root=REPO_ROOT)
...
# submits a patch; at evaluation ``-applyfail`` does not apply and ``-evalerr``
# is a harness error without a verdict. Terminal-Bench names use ``-fail``
# (reward 0), ``-timeout`` (AgentTimeoutError, no verdict), ``-envfail``
# (retryable infrastructure failure) and ``-vtimeout`` (verifier timeout whose
# reward file is recovered).
...
SWE_TASKS_BY_REPO = {
    "django": [
        ...
        "django__django-10004-applyfail",
    ],
    "sympy": [
        "sympy__sympy-20001-crash",
        "sympy__sympy-20002",
        "sympy__sympy-20003",
        "sympy__sympy-20004-evalerr",
    ],
    "scikit-learn": [
...
TB_TASKS = [
    ...
    "gamma-task-timeout",
    "delta-task",
    "epsilon-task-envfail",
    "zeta-task-vtimeout",
]
TB_TASK_LIST = {"tasks": ["beta-task-fail", "alpha-task", "zeta-task-vtimeout"]}
TB_TASK_LIST_BAD = ["alpha-task", "omega-task"]
...
"""
_MODEL_C = """model:
  model_name: "hosted_vllm/test/model-c"
  model_class: "litellm_textbased"
  model_kwargs:
...
CONFIGS = {
    ...
    # Summarizer overrides: only their `model:` section is read.
    "config-summary-b.yaml": _MODEL_B,
    "config-summary-c.yaml": _MODEL_C,
}
...
class Sandbox:
    ...
    def script(self, name: str) -> Path:
        """Locate ``scripts/<name>`` or ``scripts/<group>/<name>`` in this sandbox.

        The reorganization grouped launchers into sub-directories; a scenario
        names the script only, so the same command line runs on both sides.
        """
        candidates = [self.root / "scripts" / name]
        candidates += sorted(self.root.glob(f"scripts/*/{name}"))
...
def build_sandbox(root: Path, tree: Tree, *, python: str | None = None) -> Sandbox:
    ...
    for name in ROOT_MODULES:
        if (tree.root / name).is_file():
            shutil.copy2(tree.root / name, root / name)
