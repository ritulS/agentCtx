...
The adapter lives at `src/agentctx/benchmarks/harbor_adapter.py` (`agentctx.benchmarks.harbor_adapter:CompressionAgent`).
...
```bash
venv-harbor/bin/python -m pytest tests/test_harbor_cancellation.py -v
```
...
```bash
venv-harbor/bin/python scripts/harbor/check_vllm_cancellation.py glm
venv-harbor/bin/python scripts/harbor/check_vllm_cancellation.py devstral
```
