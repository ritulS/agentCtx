...
#
# Usage:  bash scripts/start_vllm_summarizer.sh qwen35-9b
#         bash scripts/start_vllm_summarizer.sh gemma4-12b
# Tail:   tail -f logs/vllm_summarizer_<name>.log
# Stop:   bash scripts/stop_vllm.sh logs/vllm_summarizer_<name>.pid
...
if (( free_mib < need_mib )); then
    ...
    echo "        Processes holding GPU memory (stop leftovers with scripts/stop_vllm.sh <pid file>):" >&2
