...
# scripts/run_qwen_swe_prefix_cache_ablation.sh (5.a), which turns caching ON
# for SWE-Bench, whose production runs had it off:
#   Agent      : Qwen3.5-35B-A3B, configs/config-qwen-vllm.yaml, :8000
#                server: bash scripts/start_vllm_qwen35_no_prefix_cache.sh
...
#
# Usage: nohup bash scripts/run_qwen_tb_prefix_cache_ablation.sh > logs/followup_tb_qwen_noprefixcache.nohup.log 2>&1 &
...
#     ICLR_MODEL=qwen35b-noprefixcache-smoke bash scripts/run_qwen_tb_prefix_cache_ablation.sh
...
if [[ " $server_cmdline " != *" --no-enable-prefix-caching "* || " $server_cmdline " == *" --enable-prefix-caching "* ]]; then
    ...
    echo "Restart it with: bash scripts/stop_vllm.sh logs/vllm_qwen35.pid &&" \
         "bash scripts/start_vllm_qwen35_no_prefix_cache.sh" >&2
    exit 1
fi
