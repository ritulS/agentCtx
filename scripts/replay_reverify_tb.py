...
Terminal-Bench counterpart of scripts/reevaluate_swebench_candidates.py. The
generation is left untouched: the saved trajectory's commands are re-executed
in a fresh task container by scripts/bench_adapters/replay_agent.py (no model
...
Typical sequence:
    ...
    python3 scripts/replay_reverify_tb.py plan --out logs/replay_reverify/candidates.json
    python3 scripts/replay_reverify_tb.py recover \\
        --candidates logs/replay_reverify/candidates.json --output-dir logs/replay_reverify/recover1
    venv-harbor/bin/python scripts/replay_reverify_tb.py run \\
        --candidates logs/replay_reverify/candidates.json \\
        --output-dir logs/replay_reverify/run1 --limit 1        # smoke test first
    python3 scripts/replay_reverify_tb.py apply --output-dir logs/replay_reverify/run1 [--write]
"""
...
from bench_adapters.tb_verdict import (  # noqa: E402
...
REPLAY_AGENT = "scripts.bench_adapters.replay_agent:ReplayAgent"
...
# raw_trial_dir / reward_file_evidence live in bench_adapters.tb_verdict so the
