...
# the summarizer ablation (FOLLOWUP_EXPERIMENTS.md §4). Consumed by
# configs/config-summary-qwen35-9b.yaml.
#
# GPU placement: on Dobby (4x A100 80GB) the Qwen3.5-35B-A3B agent server
# (vllm-qwen35, :8000, TP=4) normally occupies every GPU at vLLM's default 0.9
# memory utilization, so the summarizer cannot start beside it as-is. Either
# restart the agent server with a lower --gpu-memory-utilization, or run the
# summarizer on other GPUs (QWEN9B_CUDA_VISIBLE_DEVICES) and cap its share
# with QWEN9B_GPU_MEMORY_UTILIZATION. Defaults: GPUs 0-1, TP=2, 0.25.
#
...
CUDA_DEVICES="${QWEN9B_CUDA_VISIBLE_DEVICES:-0,1}"
...
# Fraction of each GPU's memory vLLM may take (it shares the GPUs with the agent).
GPU_MEM_UTIL="${QWEN9B_GPU_MEMORY_UTILIZATION:-0.25}"
